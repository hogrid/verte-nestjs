...
export class AuthService {
  ...
  async login(loginDto: LoginDto) {
    ...
    // Disparar sincronização automática de contatos (não bloquear login)
    void this.contactsService.syncFromEvolution(user.id).catch(() => undefined);
  ...
  private async buildUserResponse(user: User) {
    ...
    const numberActiveConnected = numberActive.status_connection === 1 ? 1 : 0;
    const numbersConnected = await this.numberRepository.count({
      where: { user_id: user.id, status_connection: 1 },
    });
