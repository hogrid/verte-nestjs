...
describe('WhatsApp Webhooks (e2e) - WAHA Event Processing', () => {
  ...
  async function createTestData() {
    ...
    const hasPbcCampaignId = await columnExists(
      'public_by_contacts',
      'campaign_id',
    );
    const hasMbcCampaignId = await columnExists(
      'message_by_contacts',
      'campaign_id',
    );
    ...
    const hasMbcDelivered = await columnExists(
      'message_by_contacts',
      'delivered',
    );
    ...
    const hasPbcHasError = await columnExists(
      'public_by_contacts',
      'has_error',
    );
    ...
    const [msgIdRow] = await dataSource
      .query(
    ...
    const testMessageId = msgIdRow.id;
